...
export function machineLoad(masters, data, days = 30) {
  ...
  const monthlyFixed = E.monthlyOperator + E.monthlyRent + E.monthlyElectricity + E.monthlyMaintenance
    + E.machineCost / E.lifeYears / 12
  const inhousePerPiece = moPieces > 0 ? round2(monthlyFixed / moPieces) : null
  ...
  else if (cheaperInhouse && fill1 >= 80) { verdict = 'buy'; reason = 'Machine would run near-full and in-house is cheaper per piece.' }
